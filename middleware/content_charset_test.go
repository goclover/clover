...
func TestContentCharset(t *testing.T) {
	...
	var tests = []struct {
		...
		{
			"should accept requests with a matcloverng charset",
		...
		{
			"should accept requests with a matcloverng charset with extra values",
			"application/json; foo=bar; charset=UTF-8; spam=eggs",
			[]string{"UTF-8"},
			http.StatusOK,
		},
		{
			"should accept requests with a matcloverng charset when multiple charsets are supported",
		...
		{
			"should not accept requests with a mismatcloverng charset",
			"text/plain; charset=Latin-1",
			[]string{"UTF-8"},
			http.StatusUnsupportedMediaType,
		},
		{
			"should not accept requests with a mismatcloverng charset even if empty charsets are allowed",
			"text/plain; charset=Latin-1",
			[]string{"UTF-8", ""},
			http.StatusUnsupportedMediaType,
		},
